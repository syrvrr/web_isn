...
class Model_app extends CI_model{
   ...
   public function view_result($table){
    ...
    function berita_hot_1($table,$table2,$join1,$join2,$col1,$col2,$ordering,$limit){
        return $this->db->query("SELECT * FROM $table a join $table2 b on a.$join1 = b.$join2 order by $col1,$col2 $ordering limit $limit ")->result_array();
    }

    // function berita_hot_2($table,$table2,$join1,$join2,$where,$col1,$col2,$ordering,$limit){
    //     return $this->db->query("SELECT * FROM $table a join $table2 b on a.$join1 = b.$join2 where $where not in (select max($where) from $table)  order by $col1,$col2 $ordering limit $limit ")->result_array();
    // }

     function berita_hot_2($table,$table2,$join1,$join2,$where,$col1,$col2,$ordering,$limit){
        return $this->db->query("SELECT * FROM $table a join $table2 b on a.$join1 = b.$join2 where $where != (select $where from $table order by $col1,$col2 $ordering limit 1)  order by $col1,$col2 $ordering limit $limit ")->result_array();
    }
    ...
    public function cek_login_ret($username,$password,$table){
...
function list_content($table1,$field,$where,$field2,$where2,$order,$ordering){

   
            
             $hasil=$this->db->query("select * from $table1 where $field = '$where' and $field2 != '$where2'  order by $order $ordering");
